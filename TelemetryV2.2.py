...
def create_login_page():
    global frame, usernameGet, successLabel, buttonExit, versionLabel, logoIAUPack, logoRnD, logoSYAE, logoSYAEPack, logoRnDPack, passwordGet, error_label, TelemetryLabel, copyrightLabel, buttonLogin, author_label, versionTeresa, logoIAU
    ...
    versionLabel = customtkinter.CTkLabel(master=frame, text="version 2.2 — stable-beta (under development)", font=("Inter", 10))
...
def login() -> None:
    ...
    if entered_password in userPWD:
        ...
        if userPWD[entered_password] == entered_username:
            #user_id = userID.get(entered_username)
            #print("success")
            #error_label.configure(text="Verified credential.") 
            #time.sleep(3)
            show_main_page()
