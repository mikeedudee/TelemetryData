...
APOGEE = 10000  # Maximum altitude in meters
DURATION = 60  # Total duration of the simulation in seconds
...
def generate_sample_data(t):
    ...
    pressure = 1013 * np.exp(-0.00012 * altitude)  # Approximate atmospheric pressure
    air_density = 1.225 * np.exp(-0.00012 * altitude)  # Approximate air density
...
def update(frame):
    global time_data  # Declare time_data as global
    current_time = time.time() - start_time
    elapsed_time = current_time % DURATION  # Loop over duration
