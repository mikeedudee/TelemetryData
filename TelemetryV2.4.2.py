...
class MainWindow:
    def show_main_page():
        ...
        console_text           = customtkinter.CTkTextbox(master=button_frame, wrap=customtkinter.WORD, width=450, height=70)
        run_button             = customtkinter.CTkButton(master=button_frame, text="Run Script", command=ConsoleRedirector.run_script)
...
class TimerClass:
    ...
    def update_countdown():
        ...
        if countdown_time_remaining > 1:
            ...
            if countdown_time_remaining == 10:
                ...
                InputBehaviorContainers.ConsoleTextHandler(1002)
                InputBehaviorContainers.ConsolTextDisplayHandler()
            elif countdown_time_remaining == 8:
                consoleTextLabelHandler.destroy()
                InputBehaviorContainers.ConsoleTextHandler(1003)
                InputBehaviorContainers.ConsolTextDisplayHandler()
            elif countdown_time_remaining == 7:
                consoleTextLabelHandler.destroy()
                InputBehaviorContainers.ConsoleTextHandler(1004)
                InputBehaviorContainers.ConsolTextDisplayHandler()
            elif countdown_time_remaining == 5:
                consoleTextLabelHandler.destroy()
                InputBehaviorContainers.ConsoleTextHandler(1005)
                InputBehaviorContainers.ConsolTextDisplayHandler()
            elif countdown_time_remaining == 1:
                consoleTextLabelHandler.destroy()
                InputBehaviorContainers.ConsoleTextHandler(1007)
                InputBehaviorContainers.ConsolTextDisplayHandler()
        else:
    ...
    def start_countdown():
        ...
        InputBehaviorContainers.ConsoleTextHandler(1001) #Code for external system check 
        InputBehaviorContainers.ConsolTextDisplayHandler()
...
class InputBehaviorContainers:
    #def ConsolTextDisplayHandler():
    #        global textHandled, consoleTextLabelHandler
    #         consoleTextLabelHandler = customtkinter.CTkLabel(master=button_frame, text=textHandled, text_color="green", font=("Inter", 11))
    #        consoleTextLabelHandler.grid(row=0, column=0, columnspan=5, pady=0, padx=10, sticky='ne')
    ...
    def ConsoleTextHandler(value_id):
        ...
        global textHandled 
        textHandled = InputBehaviorContainers.TextDictionaryHandler(value_id)
        print(textHandled)

    # Handle logout and return to login page
    def return_to_login():
        """
        Handles the logout process and returns the user to the login page.

        This function destroys all widgets in the main application window,
        including the plot frame and the main frame, and then recreates the login page.
        """
...
class ConsoleRedirector:
    ...
    def run_script():
        ...
        global console_text
        script_path = "ModuleTesting.py"  # Replace with your script path
