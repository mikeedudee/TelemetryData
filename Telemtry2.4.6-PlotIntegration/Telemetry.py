...
def speak(text: str) -> None:
    languageSpeaker.say(text)
    languageSpeaker.runAndWait()
...
# Function for plotting
    def generate_sample_data(t):
        data = GenerationOfSimulationData.readData()
        global altitude
        if data:
            ...
            attitude     = (float(data[12]), float(data[13]), float(data[14]))
...
if data:
    ...
    pressure        = data[1]
    altitude        = data[2]
    GyroSensorYaw   = data[12]
    GyroSensorPitch = data[13]
    GyroSensorRoll  = data[14]
    print(f"Temperature: {temperature}C | Pressure: {pressure}Pa | Altitude: {altitude}m | Yaw: {GyroSensorYaw} | Pitch: {GyroSensorPitch} | Roll: {GyroSensorRoll}")
'''
...
class LoginFrame:
    ...
    def create_login_page():
        ...
        versionLabel   = customtkinter.CTkLabel(master=frame, text="version 2.4.6 — stable-beta (under development)", font=("Inter", 10))
...
class MainWindow:
    def show_main_page():
        ...
        for i, ax in enumerate(axes.flat):
            ...
            ax.set_ylim(0, 15000 if i == 0 else 100)  # Adjust for high altitude and pressure
            ax.grid(True)
        ...
        def update(frame):
            ...
            for i, ax in enumerate(axes.flat):
                ...
                ax.set_ylim(0, max(np.max(data_buffers[list(data_buffers.keys())[i]]), 15000 if i == 0 else 1000))
...
class TimerClass:
    ...
    def update_countdown():
        ...
        if countdown_time_remaining > 1:
            ...
            elif countdown_time_remaining == 0:
